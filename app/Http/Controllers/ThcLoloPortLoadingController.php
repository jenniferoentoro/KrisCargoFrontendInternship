...
class ThcLoloPortLoadingController extends Controller
{
    public function dataTableAdvJson(Request $request)
    {
        ...
        $url = env('API_URL') . "/thc-lolo-port-loading/json-adv";
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading/json";
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading" . "/" . $request->KODE; // API endpoint to fetch provinces by KODE
    ...
    public function index(Request $request)
    {
        ...
        // $url = env('API_URL') . "/thc-lolo-port-loading";
        ...
        if ($length > 0) {
            ...
            return view('pages.1master.8hargalcl.thclolopol', [
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading" . "/" . $request->KODE;
        //post with token
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading/get-next-id";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading/trash";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to restore trash data from Route::post('/thc-lolo-port-loading/restore', [cityController::class, 'restore'])->name('city.restore');
    public function restore(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo-port-loading/restore/" . $request->KODE;
        //post with token
        $response = Http::withToken($providedToken)->post($url);
        $data = $response->json();
        // dd($data);
