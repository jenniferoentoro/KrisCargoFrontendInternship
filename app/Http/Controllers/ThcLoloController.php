...
class ThcLoloController extends Controller
{
    ...
    public function dataTableAdvJson(Request $request)
    {
        ...
        $url = env('API_URL') . "/thc-lolo/json-adv";
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo" . "/" . $request->KODE; // API endpoint to fetch provinces by KODE
    ...
    public function index(Request $request)
    {
        ...
        if ($length > 0) {
            ...
            return view('pages.1master.8hargalcl.thclolo', [
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo" . "/" . $request->KODE;
        //post with token
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo/get-next-id";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo/trash";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to restore trash data from Route::post('/thc-lolo/restore', [cityController::class, 'restore'])->name('city.restore');
    public function restore(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/thc-lolo/restore/" . $request->KODE;
        //post with token
        $response = Http::withToken($providedToken)->post($url);
        $data = $response->json();
        // dd($data);
