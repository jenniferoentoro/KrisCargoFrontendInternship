...
class TruckController extends Controller
{
    public function dataTableAdvJson(Request $request)
    {
        ...
        $url = env('API_URL') . "/truck/json-adv";
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/truck" . "/" . $request->KODE; // API endpoint to fetch provinces by KODE
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/truck" . "/" . $request->KODE;
        //post with token
    ...
    public function index(Request $request)
    {
        ...
        if ($length > 0) {
            ...
            return view('pages.1master.4truck.truck', ['type_menu' => 'layout-master', 'type_submenu' => 'layout-truck', 'data' => $data['data'], 'keys' => $columns_shown, 'table_headers' => $table_headers]);
        } else {
            return view('pages.1master.4truck.truck', ['type_menu' => 'layout-master', 'type_submenu' => 'layout-truck', 'data' => [], 'keys' => $columns_shown, 'table_headers' => $table_headers]);
        }
    }
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/truck/get-next-id";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to get trash data from Route::delete('/truck/trash', [NegaraController::class, 'trash'])->name('negara.trash');
    public function trash(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/truck/trash";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to restore trash data from Route::post('/truck/restore', [NegaraController::class, 'restore'])->name('negara.restore');
    public function restore(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/truck/restore/" . $request->KODE;
        //post with token
