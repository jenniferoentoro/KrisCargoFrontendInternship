...
class JOAController extends Controller
{
    public function findByKode(Request $request)
    {
        ...
        $url = env('API_URL') . "/joa" . "/" . $request->KODE; // API endpoint to fetch provinces by KODE
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/joa" . "/" . $request->KODE;
        //post with token
    ...
    public function index(Request $request)
    {
        ...
        $table_headers = config('joa.table_headers');
        $columns_shown = config('joa.columns_shown');

        $url_all_dropdown = env('API_URL') . "/joa/dropdown";
        ...
        if ($length > 0) {
            ...
            return view('pages.2joa.5joa.joa', [
    ...
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/joa/get-next-id";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to get trash data from Route::delete('/joa/trash', [NegaraController::class, 'trash'])->name('negara.trash');
    public function trash(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/joa/trash";
        //post with token
        $response = Http::withToken($providedToken)->get($url);
        $data = $response->json();
        // dd($data);
    ...
    // function to restore trash data from Route::post('/joa/restore', [NegaraController::class, 'restore'])->name('negara.restore');
    public function restore(Request $request)
    {
        $providedToken = $request->cookie('token');
        $url = env('API_URL') . "/joa/restore/" . $request->KODE;
        //post with token
