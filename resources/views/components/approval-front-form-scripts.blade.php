...
<script>
    ...
    const approvebutton = document.getElementById('lol');


    approvebutton.addEventListener('click', function() {
        openLoadingModal();
        $('#texttrashbin').html("Approved Data");
        $.ajax({
                ...
                if (response.success) {
                    ...
                    response.data.forEach(function(item) {
                        ...
                        var newRow = '<tr class="border-bottom" id="' + (item.KODE ? item.KODE :
                                item.NOMOR) + '">' +
                            '<td>' +
                            '<button class="btn btn-primary btn-restore" id="restorr-' +
                            (item.KODE ? item.KODE : item.NOMOR) +
                            '">Batal approve</button>' +
                            '</td>' + rowContent +
                            '</tr>';
    ...
    $(document).ready(function() {
        $(document).on('click', '.btn-restore', function() {
            ...
            var confirmation = confirm(
                "Are you sure you want to cancel the approvement data with KODE: " +
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        var newRow = [];
                        newRow.push(
                            '<button class="btn btn-success btn-sm btn-delete" id="delett-' +
                            rowData[0] + '"><i class="fa-solid fa-check"></i></button>');
    ...
    $(document).ready(function() {
        ...
        $('#clueList tbody').on('click', 'button.btn-delete', function() {
            ...
            // substring from 0 to 6
            var delete_route = id.substring(0, 6);
            ...
            var tanya = "delete";
            var method = 'DELETE';
            if (delete_route == "delett") {
                tanya = "approve";
                method = 'PUT';
            }

            var confirmation = confirm(
                "Are you sure you want to " + tanya + " data with KODE: " +
                clueKODE + " and NAMA: " + clueNAMA + "?"
            );
