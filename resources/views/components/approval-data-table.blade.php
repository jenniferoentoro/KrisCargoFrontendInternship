                    ...
                    <tr class="border-bottom" id="{{ $item['KODE'] ?? $item['NOMOR'] }}">
                        <td class="edit-column d-flex">
                            {{-- <button class="btn btn-info btn-sm mr-1 btn-edit"
                                id="edit-{{ $item['KODE'] ?? $item['NOMOR'] }}"><i class="fa-solid fa-eye"></i></button> --}}
                            <button class="btn btn-success btn-sm btn-delete"
                                id="delett-{{ $item['KODE'] ?? $item['NOMOR'] }}"><i class="fa-solid fa-check"></i></button>
                        </td>
                        {{-- for each keys --}}
                        @foreach ($keys as $key)
                            @if ($item[$key] === true)
